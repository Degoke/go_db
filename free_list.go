...
func (fl *FreeList) Update(popn int, freed []uint64) {
	...
	for fl.head != 0 && len(reuse)*FREE_LIST_CAP < len(freed) {
		...
		if popn >= flnSize(node) {
			// remove all pointers to this node
			popn -= flnSize(node)
		} else {
			...
			// reuse pointers form the free list itself
...
func flPush(fl *FreeList, freed []uint64, reuse []uint64) {
	for len(freed) > 0 {
		...
		if len(reuse) > 0 {
			// reuse a pointer form the list
			fl.head, reuse = reuse[0], reuse[1:]
			fl.use(fl.head, new)
		} else {
			// or append a page to house a new node
			fl.head = fl.new(new)
		}
	}
	assert(len(reuse) == 0)
